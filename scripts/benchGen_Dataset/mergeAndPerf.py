...
def analysePerf(
    ...
    for event, data_points in all_runs_data_raw.items():
        if not data_points:
            ...
            print(
                f"Aviso: Nenhum dado válido coletado para o evento '{event}' em '{program_name}'."
            )
        ...
        if len(data_points) >= 2:  # Pelo menos 2 pontos para calcular erro padrão e IC
            try:
                ...
                print(
                    f"Erro ao calcular IC para '{event}' em '{program_name}': {e}. Definindo como 'ERROR'."
                )
                consolidated_stats[f"{event}_ic_low"] = "ERROR"
                consolidated_stats[f"{event}_ic_high"] = "ERROR"
        else:
            print(
                f"Aviso: Poucos pontos de dados ({len(data_points)}) para calcular IC para '{event}' em '{program_name}'. Definindo como 'N/A'."
            )
            consolidated_stats[f"{event}_ic_low"] = "N/A"
            consolidated_stats[f"{event}_ic_high"] = "N/A"
    ...
    for event in perf_events.strip().split("\n"):
        header.extend(
            [f"{event}_mean", f"{event}_std_dev", f"{event}_ic_low", f"{event}_ic_high"]
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Compile, merge all source files and extract perf data given a list of programs"
    )
    parser.add_argument(
        "input_file",
        help="Text file containing the list of programs paths to be compiled and merged",
    )
